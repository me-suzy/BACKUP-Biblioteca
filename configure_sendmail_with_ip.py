...
"""
        ...
        cmd = f'echo "{escaped}" | /bin/mail -s "[Test IP] Sendmail cu IP - {timestamp}" ioan.fantanaru@gmail.com 2>&1'
        ...
        out, _ = ssh_exec(ssh, f"tail -n 20 /var/log/maillog 2>/dev/null | grep 'ioan.fantanaru' | tail -3", show=False)
        ...
        print(f"   - In loc de: root@bariasi.academia.iasi.ro")
        print()
        print(">> BACKUP CREAT:")
        print(f"   - {backup_file}")
        print()
        print(">> TEST TRIMIS:")
        print("   - To: ioan.fantanaru@gmail.com")
