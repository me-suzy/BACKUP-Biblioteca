#!/usr/bin/env python
"""
Debug email delivery pentru contact@neculaifantanaru.com
"""
...
TEST_TO = "contact@neculaifantanaru.com"
...
def main():
    print("=" * 70)
    print("   DEBUG EMAIL DELIVERY - contact@neculaifantanaru.com")
    print("=" * 70)
    print()
    ...
    try:
        ...
        print("[INFO] Testez conectivitatea la mail.neculaifantanaru.com...")
        out, _ = ssh_exec(ssh, "telnet mail.neculaifantanaru.com 25 2>&1 | head -5", show=False)
        if out:
        ...
        out, _ = ssh_exec(ssh, "nslookup mail.neculaifantanaru.com 2>&1", show=False)
        if out:
            print("DNS pentru mail.neculaifantanaru.com:")
            print(out)
        ...
        test_msg = f"""From: root@bariasi.academia.iasi.ro
To: {TEST_TO}
Subject: [DEBUG] Test email delivery - {timestamp}
Date: {datetime.now().strftime('%a, %d %b %Y %H:%M:%S +0200')}
Message-ID: <debug-{timestamp.replace(' ', '').replace(':', '').replace('-', '')}@bariasi.academia.iasi.ro>
...
Timestamp: {timestamp}
Server: bariasi.academia.iasi.ro
...
"""
        ...
        print("[INFO] Verific SPF record pentru bariasi.academia.iasi.ro...")
        out, _ = ssh_exec(ssh, "host -t TXT bariasi.academia.iasi.ro 2>&1 | grep -i spf", show=False)
        if out:
            print("SPF Record:")
            print(out)
        else:
            print("[WARN] Nu am gasit SPF record pentru bariasi.academia.iasi.ro")
