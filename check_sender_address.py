...
To: ioan.fantanaru@gmail.com
Subject: [INFO] Adresa expeditor - {timestamp}
Date: {datetime.now().strftime('%a, %d %b %Y %H:%M:%S +0200')}
Message-ID: <sender-test-{timestamp.replace(' ', '').replace(':', '').replace('-', '')}@bariasi.academia.iasi.ro>
...
"""
        ...
        # Trimit emailul
        cmd2 = f'/bin/sendmail -v ioan.fantanaru@gmail.com < /tmp/sender_test.txt 2>&1'
        
        print(f"[INFO] Trimitere email cu informatiile despre adresa expeditor...")
        print(f"[INFO] To: ioan.fantanaru@gmail.com")
        ...
        out, _ = ssh_exec(ssh, f"tail -n 20 /var/log/maillog 2>/dev/null | grep 'ioan.fantanaru@gmail.com' | tail -3", show=False)
        ...
        print("Verifica inbox-ul pentru ioan.fantanaru@gmail.com")
        print()
